...
const App: React.FC = () => {
  ...
  const sidePanelVisible = !isMobile || isSidePanelOpen;
  ...
  return (
    <ErrorBoundary>
      <div className={`h-screen w-screen font-sans bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 overflow-hidden ${isMobile && sidePanelVisible ? 'overflow-hidden' : ''}`}>
      <PWAInstallPrompt />
      ...
      <div className={`main-content transition-all duration-300 ease-in-out ${!isMobile && sidePanelVisible ? 'ml-[28rem]' : ''}`}>
        <ChatPanel
      ...
      <div className={`${isMobile ? 'fixed top-0 left-0 h-full z-30' : ''}`}>
        <SidePanel
